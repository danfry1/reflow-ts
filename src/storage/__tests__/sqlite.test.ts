...
describe('SQLiteStorage', () => {
  ...
  describe('schedules', () => {
    ...
    it('registers a schedule and reads it back', async () => {
      const stored = await storage.upsertSchedule(makeSchedule())

      expect(stored.key).toBe('nightly')
      expect(stored.input).toStrictEqual({ olderThanDays: 30 })
      expect(await storage.listSchedules()).toHaveLength(1)
    })

    it('preserves the existing cadence when re-registered with the same interval', async () => {
      ...
      // The input updates, the cadence does not — otherwise a redeploy loop
      // could push the next firing out indefinitely.
      expect(second.nextRunAt).toBe(first.nextRunAt)
      expect(second.input).toStrictEqual({ olderThanDays: 90 })
    })

    it('resets the cadence when the interval changes', async () => {
      ...
      expect(changed.nextRunAt).toBe(900_000)
      expect(changed.intervalMs).toBe(120_000)
    })

    it('claims a due schedule and advances it past now in one step', async () => {
      await storage.upsertSchedule(makeSchedule({ nextRunAt: 1_000, intervalMs: 100 }))

      const claimed = expectPresent(await storage.claimDueSchedule(['test'], 1_250))

      // The claim reports the occurrence it fired for...
      expect(claimed.nextRunAt).toBe(1_000)
      // ...and the stored schedule has already moved past `now`.
    ...
    it('does not claim a schedule that is not yet due', async () => {
      ...
      expect(await storage.claimDueSchedule(['test'], 9_999)).toBeNull()
    })

    it('claims a due occurrence exactly once', async () => {
      await storage.upsertSchedule(makeSchedule({ nextRunAt: 1_000, intervalMs: 100_000 }))

      expect(await storage.claimDueSchedule(['test'], 1_000)).not.toBeNull()
      expect(await storage.claimDueSchedule(['test'], 1_000)).toBeNull()
    })

    it('only claims schedules for the given workflows', async () => {
      await storage.upsertSchedule(makeSchedule({ key: 'other', workflow: 'elsewhere', nextRunAt: 1_000 }))

      expect(await storage.claimDueSchedule(['test'], 5_000)).toBeNull()
      // Still pending for a worker that can serve it.
      expect(await storage.claimDueSchedule(['elsewhere'], 5_000)).not.toBeNull()
    })

    it('returns null when no workflow names are given', async () => {
      await storage.upsertSchedule(makeSchedule({ nextRunAt: 1_000 }))

      expect(await storage.claimDueSchedule([], 5_000)).toBeNull()
    })

    it('deletes a schedule, reporting whether it existed', async () => {
    ...
    it('lists schedules ordered by key', async () => {
      ...
      expect((await storage.listSchedules()).map((s) => s.key)).toStrictEqual(['a', 'b', 'c'])
    })
