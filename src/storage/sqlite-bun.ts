...
export class SQLiteStorage implements StorageAdapter {
  ...
  async upsertSchedule(schedule: WorkflowSchedule): Promise<WorkflowSchedule> {
    const upsert = this.db.transaction(() => {
      ...
      return this.db
        ...
        .get(schedule.key) as unknown as WorkflowScheduleRow
    })
  ...
  async claimDueSchedule(
    ...
    const claim = this.db.transaction(() => {
      const row = this.db.prepare(`SELECT * FROM workflow_schedules
         ...
         ORDER BY next_run_at ASC, key ASC LIMIT 1`).get(now, ...workflowNames) as unknown as WorkflowScheduleRow | undefined
      if (!row) {
        return null
      }
  ...
  async listSchedules(): Promise<WorkflowSchedule[]> {
    const rows = this.db
      ...
      .all() as unknown as WorkflowScheduleRow[]
