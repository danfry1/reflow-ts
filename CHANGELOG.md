...
webhook, skip work it doesn't need, and run on a schedule that survives the
process — with the storage contract those rest on held to a single shared test
suite across every adapter.
...
- **Benchmark suite** — `bun run bench` (`benchmarks/`) measures engine + storage throughput (enqueue and end-to-end execution) for the in-memory and Bun SQLite adapters, with published baseline numbers in the README.
...
- **Breaking:** `StorageAdapter` gains two methods, `listRuns(filter?)` and `requeueRun(runId)`, implemented across all built-in adapters (`memory`, `sqlite-node`, `sqlite-bun`, `sqlite-node-builtin`). Custom adapters must implement them.
