...
- `StorageAdapter` gains two methods, `listRuns(filter?)` and `requeueRun(runId)`, implemented across all built-in adapters (`memory`, `sqlite-node`, `sqlite-bun`, `sqlite-node-builtin`). Custom adapters must implement them.
