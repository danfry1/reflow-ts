...
- **`engine.listRuns(filter?)`** — list runs for inspection and dead-letter visibility, most recent first. Filter by `status` and/or `workflow`, and paginate with `limit` and a `before` (`createdAt`) cursor. New exported `ListRunsFilter` type.
- **`engine.resume(runId)`** — re-queue a `failed` or `cancelled` run. Completed steps are preserved and skipped on replay, so execution resumes at the step that failed (its result is discarded and re-run). Returns `false` when the run is not in a resumable state.
